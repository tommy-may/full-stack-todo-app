...
type HTTPErrorResponseMessage<E extends HTTPErrorName> = E extends 'NotFound' ? NotFoundMessage : string;
type NotFoundMessage = { message: string; url: string };
...
class HTTPErrorResponse<E extends HTTPErrorName, D> extends Error {
  ...
  constructor(error: E, message: HTTPErrorResponseMessage<E>, options?: HTTPErrorResponseOptions<D>);
  constructor(error: E, message: unknown, { data, ...options }: HTTPErrorResponseOptions<D> = {}) {
    ...
    if (error === 'NotFound') {
      _.message = (message as HTTPErrorResponseMessage<'NotFound'>).message;
      _.data = { ..._.data, url: (message as HTTPErrorResponseMessage<'NotFound'>).url };
    }
